...
import { Play, Pause, SkipForward, Volume2, Check, Circle } from "lucide-react";
...
const RoundTimer = ({ 
  ...
  for (let i = 1; i <= totalRounds; i++) {
    ...
    const isPending = i > currentRound;
    ...
    roundIndicators.push(
      <div
        ...
        title={isCompleted ? `Ronda ${i} completada` : isCurrent ? `Ronda ${i} actual` : `Ronda ${i} pendiente`}
      >
        {isCompleted ? (
      ...
      <CardContent className="p-6">
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            {/* Show advance button when round is finished OR when admin wants to skip */}
            {(isFinished || hasStarted) && !isLastRound && onAdvanceRound && (
              <Button 
                variant={isFinished ? "hero" : "outline"} 
                size="lg" 
          ...
          {isFinished && isLastRound && (
            <p className="text-primary font-medium text-center">
              ¡Todas las rondas completadas! Los participantes pueden seguir enviando selecciones.
            </p>
          )}
        </div>
      </CardContent>
